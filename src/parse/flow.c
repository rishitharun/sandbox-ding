...
extern BinarySearchTree* symbol_tree;
extern BinarySearchTree* string_member_tree;
  ...
  switch(node_type)
  {
    case EQUAL_NODE:
      ...
      if(((Node*)(lhs_node->data))->type == MEMBER_NODE)
      {
        ...
        else if(member_variable->length == 0) member_variable->length = rhs->length;
        else
        {
...
*/
              Packet * current_packet = (Packet *) packet_variable->value;
              temp_packet = current_packet;
              UserVariable* member_variable = get_set_member(current_packet, current->type, SET);
...
*/
              ...
              Packet* current_packet = (Packet *) packet_variable->value;
              UserVariable* member_variable = get_set_member(current_packet, current->type, GET);
