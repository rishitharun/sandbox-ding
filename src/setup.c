...
static BinarySearchTree* payload_tree;
...
static void setupPackets()
{
  ...
  insertNode(&payload_tree, createNode("DATA\0", __PAYLOAD_DATA, NULL));
  insertNode(&payload_tree, createNode("LENGTH\0", __PAYLOAD_LEN, NULL));
  ...
  insertNode(&packet_tree, createNode("PAYLOAD\0", __PAYLOAD, (char*) payload_tree));
}
